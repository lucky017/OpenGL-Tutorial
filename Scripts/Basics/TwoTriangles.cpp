...
int main()
{
  ...
  while(!glfwWindowShouldClose(window))
  {
    ...
    GLfloat COLOR = sin(Time)/ 2.0f + 0.6f;
    GLint RedTriangle = glGetUniformLocation(shaderprogram1, "triangleColor");  
    glUniform4f(RedTriangle, COLOR, 0.0f, 0.0f, 1.0f);  
    glBindVertexArray(VAO[0]);
    glDrawElements(GL_TRIANGLES, 3, GL_UNSIGNED_INT, 0); 
   
    glUseProgram(shaderprogram2);
    Time = glfwGetTime();
    COLOR = (sin(Time) / 2.0f) + 0.6f;
    GLint BlueTriangle = glGetUniformLocation(shaderprogram2 ,"triangleColor");
    glUniform4f(BlueTriangle, 0.0f, 0.0f, COLOR, 1.0f);
    glBindVertexArray(VAO[1]);
